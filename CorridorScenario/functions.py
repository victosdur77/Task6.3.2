...
def dibujaNubePuntosInstante(time,robotVision=None,vision_radius=5,field_of_view=np.pi/2,ids=False):
    ...
    plt.quiver(x, y, np.cos(angle), np.sin(angle), color="green", scale=30,width=0.01, headwidth=3, headlength=5)
    if ids is True:
...
def dibujaNubePuntos2Instantes(time1,time2):
    ...
    axs[0].quiver(x1, y1, np.cos(angle1), np.sin(angle1), color="red",scale=30,width=0.01, headwidth=3, headlength=5)
    ...
    axs[1].quiver(x2, y2, np.cos(angle2), np.sin(angle2), color="red", scale=30,width=0.01, headwidth=3, headlength=5)
    ...
    axs[2].quiver(x1, y1, np.cos(angle1), np.sin(angle1), color="blue", scale=30,width=0.01, headwidth=3, headlength=5, label=f'Initial time: {time1}')
    axs[2].quiver(x2, y2, np.cos(angle2), np.sin(angle2), color="red",scale=30,width=0.01, headwidth=3, headlength=5, label=f'End time: {time2}')
    for i in range(len(x1)):
